...
@Component("mintTransactionHandler")
public class MintTransactionHandler implements TransactionHandler {

    private static final Logger logger = LoggerFactory.getLogger(MintTransactionHandler.class);
    ...
    public MintTransactionHandler(CariClient cariClient,
    ...
    public CreateTransactionResponse moveFunds(Transaction transaction) {
        ...
        // Gets the internalBankCdaId to send to Cari
        AccountEntity accountEntity = accountRepository
                ...
                .orElseThrow(() -> {
                    ...
                    return new ElementNotFoundException(
                            ...
                            "CDA_NOT_FOUND",
                            "bank_cda_id",
                            "No CDA found for that bank_cda_id under your bank"
                    );
                });
        ...
        CariTransactionRequest mintRequest = mapper.toCariTransactionRequest(transaction, accountEntity.getInternalBankCdaId());

        // Calls Cari endpoint
        CariTransactionResponse cariTransactionResponse;

        try {
            cariTransactionResponse = cariClient.executeTransaction(mintRequest);
        } catch (RestClientException e) {
            // Calls Hogan to credit the account. Transaction Code 9700 is for credit
            updateAccountBalance(transaction, 9700L);
            logger.error("Error trying to mint account. {}", e.getMessage());
            throw new RestClientException("Error trying to mint account. " + transaction.getBankCdaId());
        }

        // Populates Entity with Cari Response and Persists database
        TransactionEntity transactionEntity = mapper.toTransactionEntity(transaction, cariTransactionResponse, mintRequest);
        transactionRepository.save(transactionEntity);
        logger.info("Mint transaction persisted into database: {}", transactionEntity);
